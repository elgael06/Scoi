...
class Anchor {
    ...
    onClick(ev) {
        __chunk_4.openURL(this.win, this.href, ev, this.routerDirection);
    }
    hostData() {
        const mode = __chunk_1.getIonMode(this);
        return {
            class: Object.assign({}, __chunk_4.createColorClasses(this.color), { [`${mode}`]: true, 'ion-activatable': true })
        };
    }
    __stencil_render() {
        return (__chunk_1.h("a", { href: this.href }, __chunk_1.h("slot", null)));
    }
...
class BackButton {
    ...
    async onClick(ev) {
        ...
        return __chunk_4.openURL(this.win, this.defaultHref, ev, 'back');
    }
    hostData() {
        const showBackButton = this.defaultHref !== undefined;
        const mode = __chunk_1.getIonMode(this);
        return {
            class: Object.assign({}, __chunk_4.createColorClasses(this.color), { [`${mode}`]: true, 'button': true, 'ion-activatable': true, 'show-back-button': showBackButton })
        };
    }
    __stencil_render() {
        const mode = __chunk_1.getIonMode(this);
        ...
        return (__chunk_1.h("button", { type: "button", class: "button-native" }, __chunk_1.h("span", { class: "button-inner" }, backButtonIcon && __chunk_1.h("ion-icon", { icon: backButtonIcon, lazy: false }), backButtonText && __chunk_1.h("span", { class: "button-text" }, backButtonText)), mode === 'md' && __chunk_1.h("ion-ripple-effect", { type: "unbounded" })));
    }
