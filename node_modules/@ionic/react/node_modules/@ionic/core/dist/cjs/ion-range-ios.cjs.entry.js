...
class Range {
    constructor(hostRef) {
        ...
        this.clampBounds = (value) => {
            return __chunk_6.clamp(this.min, value, this.max);
        };
        ...
        this.handleKeyboard = (knob, isIncrease) => {
            ...
            if (knob === 'A') {
                this.ratioA = __chunk_6.clamp(0, this.ratioA + step, 1);
            }
            else {
                this.ratioB = __chunk_6.clamp(0, this.ratioB + step, 1);
            }
            this.updateValue();
        };
    ...
    debounceChanged() {
        this.ionChange = __chunk_6.debounceEvent(this.ionChange, this.debounce);
    }
    minChanged() {
        if (!this.noUpdate) {
            this.updateRatio();
    ...
    onStart(detail) {
        ...
        let ratio = __chunk_6.clamp(0, (currentX - rect.left) / rect.width, 1);
        if (document.dir === 'rtl') {
            ratio = 1 - ratio;
        }
        this.pressedKnob =
    ...
    hostData() {
        ...
        return {
            class: Object.assign({}, __chunk_4.createColorClasses(this.color), { [`${mode}`]: true, 'in-item': __chunk_4.hostContext('ion-item', this.el), 'range-disabled': this.disabled, 'range-pressed': this.pressedKnob !== undefined, 'range-has-pin': this.pin })
        };
    }
    __stencil_render() {
        ...
        return [
            ...
            __chunk_1.h("div", { class: "range-slider", ref: el => this.rangeSlider = el }, ticks.map(tick => (__chunk_1.h("div", { style: tickStyle(tick), role: "presentation", class: {
                    'range-tick': true,
...
function ratioToValue(ratio, min, max, step) {
    ...
    return __chunk_6.clamp(min, value, max);
}
function valueToRatio(value, min, max) {
    return __chunk_6.clamp(0, (value - min) / (max - min), 1);
}
