...
class Button {
    ...
    onClick(ev) {
        ...
        else if (__chunk_6.hasShadowDom(this.el)) {
    ...
    hostData() {
        ...
        return {
            ...
            class: Object.assign({}, __chunk_4.createColorClasses(color), { [`${mode}`]: true, [buttonType]: true, [`${buttonType}-${expand}`]: expand !== undefined, [`${buttonType}-${size}`]: size !== undefined, [`${buttonType}-${shape}`]: shape !== undefined, [`${buttonType}-${fill}`]: true, [`${buttonType}-strong`]: strong, 'button-has-icon-only': hasIconOnly, 'button-disabled': disabled, 'ion-activatable': true, 'ion-focusable': true })
        };
    }
    __stencil_render() {
        const mode = __chunk_1.getIonMode(this);
